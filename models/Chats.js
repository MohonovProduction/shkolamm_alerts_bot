...
Chats.scene.action(/chat/, async ctx => {
    ...
    if (parameter.search('add') !== -1) {
        ctx.editMessageText(
            `Обычному пользователю нужно написать мне /start\n\nЕсли пользователь/канал/чат отписан, то он не будет отображаться в списке\n\nПросто добавь меня в чат или канал (в канал нужно отправить одно любое сообщение, чтобы я его запомнил) 😏`,
            {
                reply_markup: {
